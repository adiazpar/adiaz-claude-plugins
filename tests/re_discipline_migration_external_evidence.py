...
def _agent_prompt(request: Mapping[str, Any], condition: str) -> str:
    ...
    return (
        ...
        "the trial: always pass an explicit tokenBudget on every retrieval "
        "and read call, sized so that the result tokens already consumed by "
        "earlier calls plus this call stay within the total; a single focused "
        "retrieval with a small result limit may spend most of the remaining "
        "allowance when that is what answering takes. Use a small result "
        "limit where the schema offers one, make at most three retrieval "
        "calls, and stop retrieving the moment you can answer. "
