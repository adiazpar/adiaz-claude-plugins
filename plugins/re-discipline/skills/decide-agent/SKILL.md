...
**fire** it is a `promoted: true` provider in `agents/config.json`.
...
1. Graduate `recruiting/<candidate>/config-draft.json`'s provider entry into
   `agents/config.json` with `promoted: true`.
...
4. Update `agents/README.md`, the **`.claude/project-profile.md` Tooling roster line**, and
...
5. Write `agents/roster/<provider>/teardown-manifest.md` recording every file + location
...
Precondition: a `promoted: true` provider in `agents/config.json`. Confirm with the user
(this removes a working team member). Then replay `agents/roster/<provider>/teardown-manifest.md`
IN REVERSE:
1. Remove the provider entry from `agents/config.json`.
...
5. Archive a one-line "departed <date>" note; delete `agents/roster/<provider>/`.
6. Leave the CLI installed on the machine. Verify no residual references remain
   (`grep` the provider name across `agents/`, `CLAUDE.md`, `AGENTS.md`, README, memory).
...
- Provider config schema + dispatch mechanics: `agents/README.md`.
