...
3. **MCP registration** — make the candidate's daemon + ghidra MCP registration permanent in the
...
4. **`tools/agents/README.md`** — note the provider in the providers/roster section. *Fire:* drop
   the note.

5. **`.claude/project-profile.md` (Tooling roster line)** — the roster of promoted external agents
...
6. **Project memory** — a one-line `MEMORY.md` pointer + any provider fact file (mirrors the
   Codex bypass-policy memory). *Fire:* remove the pointer + fact file.

7. **`tools/agents/roster/<provider>/`** — created by promote: the `teardown-manifest.md` (the
   exact undo for 1–6) + a one-line hiring record. *Fire:* delete this dir last, after replaying it.
