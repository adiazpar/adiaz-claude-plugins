...
func TestDocumentSupersessionPacksReplacementBeforeObsoletePassage(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, "docs", "truth", "replacement.md"), `# Current replacement

**Claim:** The maintained contract is replacement-protocol-v2.

**Confidence:** DIRECT
`)
	writeTestFile(t, filepath.Join(root, "docs", "truth", "obsolete.md"), `# Obsolete contract
...
`)
	...
	for index, result := range search.Results {
		...
		case "docs/truth/replacement.md":
			if replacementIndex < 0 {
				replacementIndex = index
			}
		case "docs/truth/obsolete.md":
			if obsoleteIndex < 0 {
				obsoleteIndex = index
			}
...
func TestDocumentSupersessionDropsObsoletePassageWhenReplacementDoesNotFit(t *testing.T) {
	root := makeAdversarialProject(t)
	writeTestFile(t, filepath.Join(root, "docs", "truth", "large-replacement.md"),
		"# Current replacement\n\n**Claim:** "+strings.Repeat("authoritative replacement detail ", 90)+"\n")
	writeTestFile(t, filepath.Join(root, "docs", "truth", "small-obsolete.md"), `# Small obsolete contract
...
`)
	...
	for _, result := range search.Results {
		if result.Citation.Path == "docs/truth/small-obsolete.md" {
...
func TestDocumentSupersessionDropsObsoletePassageWhenReplacementIsMissing(t *testing.T) {
	root := makeAdversarialProject(t)
	writeTestFile(t, filepath.Join(root, "docs", "truth", "orphaned-obsolete.md"), `# Orphaned obsolete contract

**Superseded-by:** docs/truth/missing-replacement.md
...
`)
	...
	for _, result := range search.Results {
		if result.Citation.Path == "docs/truth/orphaned-obsolete.md" {
