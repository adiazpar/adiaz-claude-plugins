...
**Staff every dispatch by ROLE.** A *role* is a function in the workflow; an *agent* is a model that can perform roles; a profile's `role-fit` says which roles an agent is good at. You bind role→agent per task: name the role the task needs, consult the roster's `role-fit` (`agents/profiles/<agent>.md` + the profile's roster line), pick the agent. Roles are NOT standing agents — they are your staffing vocabulary.

The **generic roles** (every campaign has these):
...
| **Analyst** | the project's substantive reasoning — decompile, proof, schema, research, whatever the hard thinking is | manager tier, or a promoted external analyst |
| **Mechanical fan-out** | bulk extract / diff / enumerate / triage (output is cheaply checkable) | a lighter tier (cost lever) |
| **Synthesizer** | cross-report integration · completeness-critique · generalization | manager tier |

**Domain roles live in the profile, not here.** They are born from a project's *apparatus*, so a project declares its own in `.claude/project-profile.md` (§ Roles): a project with a live oracle adds a *live-tester*; one with visual output adds a *vision-reader*; an RE project scopes *Analyst* as an *RE-analyst*. Staff from the generic roles **plus** the project's declared roles. `init-project` seeds the obvious domain roles from the project type; the rest accrete as you build the apparatus (a new capability → a new role).
